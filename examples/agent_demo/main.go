// agent_demo shows how to build a ReAct agent using Baize (built on Graphflow).
//
...
import (
	...
	llmgate_adapter "github.com/wzhongyou/baize/agent/llmgate"
...
func main() {
	...
	ag := agent.NewReActAgent(agent.ReActAgentConfig{
		Name:         "react-agent",
		LLM:          llmModel,
		SystemPrompt: "You are Baize, a universal super-agent. You can think, reason, use tools, and solve problems across any domain — math, coding, research, writing, data analysis, and more.",
		Tools:        tools,
		MaxSteps:     20,
	})
	g, err := ag.BuildGraph()
