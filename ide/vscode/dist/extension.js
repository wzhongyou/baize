...
export function activate(context) {
    ...
    // 注册命令：打开对话面板
    context.subscriptions.push(vscode.commands.registerCommand('cangjie.openChat', () => {
        ChatPanel.createOrShow(context);
    }));
    // 注册命令：行内编辑（Cmd+K 触发）
    context.subscriptions.push(vscode.commands.registerCommand('cangjie.inlineEdit', async () => {
        ...
        const permissionCallback = async (tool, args) => {
        ...
        await vscode.window.withProgress({
            ...
            for await (const event of agentService.run(`${instruction}\n\n代码:\n\`\`\`\n${selectedText}\n\`\`\``, controller.signal, permissionCallback)) {
    ...
    // 注册命令：解释代码
    context.subscriptions.push(vscode.commands.registerCommand('cangjie.explainCode', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return;
        const selection = editor.selection;
