...
// AskFunc handles the "ask" permission decision by requesting user confirmation.
// It blocks until the user decides, then returns true (allow) or false (deny).
// Implementations may interact with a TUI, send an API event, or default to deny.
type AskFunc func(ctx context.Context, toolName string, args map[string]any, reason string) bool
...
// WithAskFunc injects an ask handler for user confirmation of "ask" decisions.
func (n *ToolNode) WithAskFunc(fn AskFunc) *ToolNode {
	n.ask = fn
	return n
}
...
func (n *ToolNode) executeToolCall(ctx context.Context, tc ToolCall) string {
	...
	if n.perm != nil {
		...
		case "ask":
			if n.ask != nil {
				if !n.ask(ctx, tc.Name, tc.Arguments, reason) {
					return fmt.Sprintf("Permission denied by user: %s", reason)
				}
				// User allowed — fall through to execute.
			} else {
				// No AskFunc configured: deny by default (safe).
				return fmt.Sprintf("Permission denied: %s (confirmation required but no handler)", reason)
			}
		}
	}
...
func (n *supervisorRouteNode) Run(ctx context.Context, s *MessageState) (*MessageState, error) {
	...
	if !ok {
		s.Messages = append(s.Messages, Message{
			Role:       RoleTool,
			Content:    fmt.Sprintf("unknown sub-agent: %s", agentName),
			ToolCallID: tc.ID,
			ToolName:   tc.Name,
			Timestamp:  time.Now(),
		})
	...
	if subErr != nil {
		s.Messages = append(s.Messages, Message{
			Role:       RoleTool,
			Content:    fmt.Sprintf("failed to build sub-agent %s: %v", agentName, subErr),
			ToolCallID: tc.ID,
			ToolName:   tc.Name,
			Timestamp:  time.Now(),
		})
	...
	if subErr != nil {
		s.Messages = append(s.Messages, Message{
			Role:       RoleTool,
			Content:    fmt.Sprintf("failed to run sub-agent %s: %v", agentName, subErr),
			ToolCallID: tc.ID,
			ToolName:   tc.Name,
			Timestamp:  time.Now(),
		})
