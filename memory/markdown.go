...
// similar to Cangjie's .cangjie/memory/ approach.
package memory
...
func (s *MarkdownStore) Search(ctx context.Context, query string, topK int) ([]api.MemoryResult, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	out := make([]api.MemoryResult, len(results))
